...
const pageLeastNum = 1; // 최소 페이지
const pageMaxNum = 3; // 최대 페이지
const pageLength = 3; // 페이지 당 사진 개수
const totalPhotoNum = todayPhoto.length;
...
function drawImages() {
    if (count !== Math.ceil(totalPhotoNum / pageLength)) {
        let str = '';
        for (let i = (count - 1) * pageLength; i < count * pageLength; i += 1) {
        ...
        let str = '';
        for (let i = (count - 1) * pageLength; i < totalPhotoNum; i += 1) {
            str += `<div class="imageBox" ><img src="${todayPhoto[i].img}" class = "image">  <div class="imageTitle">${todayPhoto[i].title}</div></div>`;
        }
        wrap.innerHTML = str;
        navNum.innerHTML = `${count.toString()} / ${pageMaxNum.toString()}`;
